<template>
    <div class="bookings-list">
      <Navbar />
      <div class="content">
        <h2>Ongoing Bookings List</h2>
        <div v-if="loading" class="loading-text">Loading...</div>
        <div v-else>
          <div v-if="ongoingBookings.length === 0" class="no-bookings-text">
            No ongoing bookings found.
          </div>
          <div v-else>
            <div class="card-container">
              <div class="card" v-for="booking in ongoingBookings" :key="booking.id">
  ...
  export default {
    ...
    computed: {
      ongoingBookings() {
        return this.bookings.filter(booking => !booking.isCompleted);
      }
    },
    mounted() {
      this.fetchBookings();
